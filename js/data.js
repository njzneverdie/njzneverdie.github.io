...
const projectsData = [
  {
    ...
    summary: {
      en: "A profile-aware AI kitchen OS: one meal idea becomes a personalized recipe, grocery list, pantry updates, hands-free guided cooking, and USDA-backed nutrition tracking — a complete plan → shop → cook → review loop. Click to open the live app.",
      zh: "會讀懂你的 AI 廚房作業系統：一個菜色想法變成個人化食譜、購物清單、庫存管理、免持引導烹飪與 USDA 佐證的營養追蹤 — 規劃、採買、烹飪、回顧完整閉環。點擊開啟線上 App。"
    },
    description: {
      en: "A personalized AI cooking companion that starts with the person, not a generic recipe — and closes the full loop from planning to review. During onboarding, users set body data, calorie/macro targets, body-composition goal, allergies, dietary needs, dislikes, and kitchen equipment; every plan is designed around that profile, with allergy/dietary-safe healthy swaps (lactose-free, gluten-free, nut-safe, shellfish-safe) carried into every meal. A Gemini-powered planner returns exact purchasable ingredients, cooking steps, and reuse ideas, honoring the user's pantry first. Whole-meal nutrition is computed against USDA FoodData Central with honest coverage disclosure, feeding a daily intake dashboard with quick logging. Grocery lists flow into the pantry after shopping, and cooked meals deduct from it automatically (with undo). A weekly planner merges a full week into one deduplicated grocery list. Chef Mode guides cooking hands-free — voice commands, spoken step announcements, per-step countdowns that survive refreshes, and screen wake lock — and every finished dish asks for a one-tap taste rating that personalizes future plans without ever overriding allergies. Bilingual (EN/繁中), installable as an offline-capable PWA. Auth, profiles, pantry, recipes, meal plans, and nutrition logs run on Supabase (RLS-protected) with Deno edge functions guarding the AI and nutrition APIs behind refundable per-user quotas. Built end-to-end with Claude Code.",
      zh: "一款從「煮的人」而非「通用食譜」出發的個人化 AI 料理助手，並把規劃到回顧的完整閉環接起來。使用者在 onboarding 設定身體數據、熱量／三大營養素目標、體態目標、過敏原、飲食需求、不喜歡的食物與廚房設備，之後每份計畫都圍繞這份 profile 設計，並把過敏與飲食安全（無乳糖、無麩質、無堅果、無甲殼類）的健康替換帶進每一餐。由 Gemini 驅動的規劃器產出可直接購買的精確食材、烹飪步驟與剩食再利用點子，並優先使用現有庫存。整餐營養以 USDA FoodData Central 實算並誠實揭露覆蓋率，串接每日攝取儀表板與快速記錄。購物清單採買完成後一鍵入庫，煮完自動扣除用掉的庫存（可復原）；一週餐期規劃器把整週餐點合併成一份去重複的購物清單。Chef Mode 支援免持烹飪 — 語音指令、步驟語音播報、逐步倒數（重新整理後保留）與螢幕恆亮 — 每道完成的料理以一鍵評分讓未來食譜更懂你，且永不凌駕過敏限制。雙語介面（英／繁中），可安裝為支援離線的 PWA。登入、個人檔案、庫存、食譜、餐期與營養記錄皆建於 Supabase（RLS 保護），並以 Deno edge function 與可退款的使用者配額保護 AI 與營養 API。全程以 Claude Code 端到端打造。"
    },
    tech: ["Supabase", "Gemini API", "Deno Edge Functions", "USDA FoodData Central", "Web Speech API", "PWA", "JavaScript"],
